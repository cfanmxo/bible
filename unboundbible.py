...
def about():
    messagebox.showinfo("About", "Unbound Bible Питон редакция \n\n Open Source Application")


def help_box():
    messagebox.showinfo("help_box", "Справка \n\n Пока нет, но скоро будет")
...
toolsmenu.add_command(label=menuFind, underline=0, accelerator='Ctrl+F', command=find)
toolsmenu.add_command(label=menuCompare, underline=0, command=compare)

themesmenu = Menu(menubar, tearoff=0)
toolsmenu.add_cascade(label=menuThemes, menu=themesmenu)
